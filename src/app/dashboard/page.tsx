...
export default function Dashboard() {
  ...
  // Function to check shop status and current time based on Indian time (IST)
  const checkShopStatus = () => {
    ...
    // Monday (day 1): 8:30 AM to 8:00 PM
    if (day === 1) {
      isOpen = (hours === 8 && minutes >= 30) || (hours > 8 && hours < 20) || (hours === 20 && minutes === 0);
    }
    // Tuesday to Saturday (day 2-6): 8:00 AM to 8:00 PM
    else if (day >= 2 && day <= 6) {
      isOpen = hours >= 8 && hours < 20;
    }
    // Sunday (day 0): 8:00 AM to 1:00 PM
    else if (day === 0) {
      isOpen = hours >= 8 && hours < 13;
    }
    
    setIsShopOpen(isOpen);
  };

  // Check shop status and update time every minute
  useEffect(() => {
    checkShopStatus();
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-100 to-blue-200 relative">
      ...
      <div className={`fixed top-0 left-0 right-0 z-50 p-4 text-center text-white font-medium text-lg transition-all duration-300 ${isShopOpen ? 'bg-green-600' : 'bg-red-600'}`}>
        {isShopOpen ? (
          ...
          <span>Shop is Closed (Open Mon: 8:30 AM - 8:00 PM, Tue-Sat: 8:00 AM - 8:00 PM, Sun: 8:00 AM - 1:00 PM IST)</span>
        )}
      </div>
